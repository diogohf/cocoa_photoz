...
void set_nuisance_pz_model(const int pz_model)
{
  cosmolike_interface::set_nuisance_pz_model(pz_model);
}

void set_nuisance_ia_nla_term(const int ia_nla_term)
{
  cosmolike_interface::set_nuisance_ia_nla_term(ia_nla_term);
}
...
// DHFS MOD START - READ SOURCE NZ PCS
  m.def("init_redshift_distributions_pcs_from_files",
      &cosmolike_interface::init_redshift_distributions_pcs_from_files,
      "Init PCs of source n(z) from file",
      py::arg("source_multihisto_pcs_file").none(false)
    );
// DHFS MOD END - READ SOURCE NZ PCS
  ...
  m.def("set_nuisance_pz_model",
      &set_nuisance_pz_model,
      "Set photoz model",
      py::arg("pz_model").none(false)
    );

  m.def("set_nuisance_ia_nla_term",
      &set_nuisance_ia_nla_term,
      "Set IA NLA term contribution",
      py::arg("ia_nla_term").none(false)
    );
  ...
  m.def("C_ss_tomo_limber_WK1WK2PK",
      py::overload_cast<const double, const int, const int>(
        &cosmolike_interface::C_ss_tomo_limber_WK1WK2PK_cpp
      ),
      "Compute shear-shear (fourier - limber) data vector at a single"
      " tomographic bin and ell value",
      py::arg("l").none(false).noconvert(),
  ...
  m.def("int_for_C_ss_tomo_limber_WK1WK2PK",
      py::overload_cast<const double, const double, const int, const int>(
        &cosmolike_interface::int_for_C_ss_tomo_limber_WK1WK2PK_cpp
      ),
      "Compute integrand for shear-shear (fourier - limber) data vector"
      " at a single tomographic bin and ell value",
      py::arg("a").none(false).noconvert(),
